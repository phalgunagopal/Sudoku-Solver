...
def solve_sudoku(board):
    ...
    if not blanks:
        ...
        print(t1-t0,"seconds was taken to solve")
        ...
        for i in range(1,10):
            if check_existence(board,i,(row,column)):
                pygame.init()
                board[row][column]=i
                value = myfont.render(str( i), True, pygame.Color("black"))
                try:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if (0 < event.key - 48 < 10):  # We are checking for valid input
                    pygame.draw.rect(screen, pygame.Color("white"), (position[1] * 80+20, position[0] * 80+20, 80-30 , 80-10 ))
                    value = myfont.render(str(event.key - 48), True, pygame.Color("black"))
                    screen.blit(value, (position[1] * 80+15+30, (position[0] * 80)+25))

                    pygame.display.update()
                    b=True
                    for k in range(len(grid)):
                        if event.key-48 == grid[position[0]][k] and position[1] != k:
                            b=False
                    for k in range(len(grid)):
                        if event.key-48 == grid[k][position[1]] and position[0] != k:
                            b=False
                    for k in range((position[0] // 3) * 3, ((position[0] // 3) * 3) + 3):
                        for m in range((position[1] // 3) * 3, ((position[1] // 3) * 3) + 3):
                            if event.key-48 == grid[k][m] and (k, m) != position:
                               b=False
                    if  not b:
                        pygame.draw.rect(screen, pygame.Color("white"),(position[1] * 80 + 20, position[0] * 80 + 20, 80 - 30, 80 - 10))
                        grid[i][j]=0
